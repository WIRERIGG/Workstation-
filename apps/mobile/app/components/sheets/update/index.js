...
export const Update = ({ version: appVersion, fwdRef }) => {
  ...
  let notes = version?.notes
    ? version.notes.replace("Thank you for using Workstation!", "").split("- ")
  ...
  const GITHUB_URL =
    !version || !version.needsUpdate
      ? null
      : `https://github.com/streetwriters/workstation/releases/download/${
          version.version
        }-android/workstation-${getSupportedAbi()}.apk`;
  const GITHUB_PAGE_URL =
    !version || !version.needsUpdate
      ? null
      : `https://github.com/streetwriters/workstation/releases/tag/${version.version}-android`;
